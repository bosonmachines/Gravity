...
class comm():
    ...
    @locked
    def connect(self, port, baud, dtr):
        if self.printer:
            self.disconnect()
        if port is not None:
            self.port = port
        ...
        if self.port is not None and self.baud is not None:
            ...
            try:
                self.printer = Serial(port = self.port, baud = self.baud, timeout = 0.25, parity = PARITY_ODD)
                self.printer.close()
    ...
    def _send(self, command, lineno = 0, calcchecksum = False):
        ...
        try:
            ...
            if e.errno is None:
                ...
                self.logError(_(u"Can't write to printer (disconnected?) (Socket error {0}): {1}").format(e.errno, e.strerror.decode('utf8')))
            self.writefailures += 1
        except SerialException as e:
            self.logError(
                _(u"Can't write to printer (disconnected?) (SerialException): {0}").format(str(e.decode('utf-8'))))
            self.writefailures += 1
        except RuntimeError as e:
            self.logError(
                _(u"Socket connection broken, disconnected. ({0}): {1}").format(e.errno, e.strerror.decode('utf-8')))
            self.writefailures += 1
